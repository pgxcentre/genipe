...
association of sets of rare and common variants. In :py:mod:`genip`, we provide
...
Input files generated by genip
^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^^
...
This file is generated by :py:mod:`genip` and has the ``.sample`` extension. It
greatly resembles the PLINK ``fam`` file for those who are familiar with this
format. Specifically, it contains the samples that are included in the
...
The `.good_sites` file is also generated by :py:mod:`genip`. It contains a
...
.. code-block:: console
    ...
    imputed-stats skat \
        --impute2 data/genip_is_great.impute2 \
        --sample data/genip_is_great.sample \
        --pheno data/phenotypes.txt \
        --sample-column Sample \
        --pheno-name SystolicBP \
        --covar Age \
        --extract-sites data/genip_is_great.good_sites \
        --nb-process 2 \
...
2. The `Impute2` file as previously described. This contains the probability
   for every allele of every variant. It is generated by :py:mod:`genip`.
3. The `sample` contained the sample identifiers with an ordering consistent
   with the `Impute2` file. It was generated by :py:mod:`genip`.
...
8. A list of variants to keep for the analysis is provided here. A good choice
   would be to simply use the `.good_sites` file generated by py:mod:`genip`,
...
You can also verify the `my_skat_analysis.log` file to see if :py:mod:`genip`
...
:py:mod:`genip` source and modify the
`run_skat.R <https://github.com/pgxcentre/genip/blob/master/genip/script_templates/run_skat.R>`_
file. This is a template rendered and launched by :py:mod:`genip` to conduct
...
`Issue tracker <https://github.com/pgxcentre/genip/issues>`_.
