...
.. code-block:: bash
   ...
   mkdir -p $HOME/genip_tutorial/logistic
...
.. code-block:: bash
   ...
   cd $HOME/genip_tutorial/logistic

   wget http://pgxcentre.github.io/genip/_static/tutorial/phenotypes_logistic.txt.bz2
...
.. code-block:: console
   ...
   distribution. This script is part of the 'genip' package, version 1.1.0).
...
.. note::
   ...
   Execution times between *Plink* and :py:mod:`genip` were compared for this
   ...
   phenotype), *Plink* was **significantly faster** than :py:mod:`genip`, even
   ...
   and keeping all samples), :py:mod:`genip` was faster with two processes or
   ...
   compiled on a modern Linux system, *i.e.* when :py:mod:`genip` is installed
...
The logistic regression results from :py:mod:`genip` and *Plink* were compared
...
shows the results from :py:mod:`genip`, and the *y* axis shows the results for
...
.. figure:: ../_static/images/Logistic_Diff_Prob0.png
   :align: center
   :width: 100%
   :alt: Logistic regression comparison between genip and Plink (prob. of 0)

.. note::

   The sign of the coefficients might be different when comparing
   :py:mod:`genip` to *Plink*, since :py:mod:`genip` computes the statistics on
...
.. note::
   ...
   :py:mod:`genip` for the same analysis, but using the default probability
   ...
   .. figure:: ../_static/images/Logistic_Diff.png
      :align: center
      :width: 100%
      :alt: Logistic regression comparison between genip and Plink
