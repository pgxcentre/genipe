...
class TestDB(unittest.TestCase):
    ...
    def setUp(self):
        ...
        self.output_dir = TemporaryDirectory(prefix="genip_test_")
    ...
    def test_create_task_db(self):
        ...
        # The only value should have name=genip_task
        self.assertEqual("genip_task", tables[0][0])
        ...
        c.execute("PRAGMA table_info(genip_task)")
        col_names = set()
    ...
    def test_create_db_connection(self):
        ...
        # Checking that the table 'genip_task' exists
        c.execute("SELECT name FROM sqlite_master WHERE type='table'")
        self.assertEqual("genip_task", c.fetchone()[0])
    ...
    def test_check_task_completion(self):
        ...
        conn, c = _create_db_connection(self.db_name)
        c.execute("UPDATE genip_task SET completed='foo' WHERE name=?",
                  (self.task_names[3], ))
        conn.commit()
        conn.close()
