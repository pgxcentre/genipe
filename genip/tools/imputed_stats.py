...
def main(args=None):
    ...
    desc = ("Performs statistical analysis on imputed data (either SKAT "
            ...
            "script is part of the 'genip' package, version "
            "{}).".format(__version__))
...
def read_sites_to_extract(i_filename):
    ...
    threshold. The :py:mod:`genip` pipeline generates this file at the
    'merge_impute2' step.
...
def _skat_generate_r_script(dir_name, r_files, args):
    ...
    jinja_env = jinja2.Environment(
        loader=jinja2.PackageLoader("genip", "script_templates")
    )
...
def check_args(args):
    ...
    Note
    ----
        If there is a problem, a :py:class:`genip.error.ProgramError` is
        raised.
...
def parse_args(parser, args=None):
    ...
    parser.add_argument(
        ...
        "--version",
        action="version",
        version="%(prog)s, part of genip version {}".format(__version__),
    )
    ...
    cox_parser = subparsers.add_parser(
        ...
        description="Performs a survival regression on imputed data using "
                    ...
                    "the 'genip' package, version {}).".format(__version__),
        parents=[p_parser],
    )
    ...
    lin_parser = subparsers.add_parser(
        ...
        description="Performs a linear regression (ordinary least squares) on "
                    "imputed data. This script is part of the 'genip' "
    ...
    skat_parser = subparsers.add_parser(
        ...
        description="Uses the SKAT R package to analyze user defined gene "
                    "sets. This script is part of the 'genip' package, "
