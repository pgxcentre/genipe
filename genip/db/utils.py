...
def create_task_db(out_dir):
    ...
    name ``tasks.db``. The ``genip_task`` table is automatically created.
    ...
    c.execute("""CREATE TABLE IF NOT EXISTS genip_task (
...
def check_task_completion(task_id, db_name):
    ...
    c.execute("SELECT completed FROM genip_task WHERE name=?", (task_id, ))
    r = c.fetchone()
...
def create_task_entry(task_id, db_name):
    ...
    c.execute("SELECT name FROM genip_task WHERE name=?", (task_id, ))
    r = c.fetchone()
    ...
    if r is None:
        ...
        c.execute("INSERT INTO genip_task (name, launch, start) "
                  "VALUES (?, ?, ?)",
                  (task_id, time, time))

    else:
        # We saw this task, but we need to relaunch it (setting completed=0)
        c.execute("UPDATE genip_task "
...
def mark_task_completed(task_id, db_name):
    ...
    c.execute("UPDATE genip_task SET end=?, completed=1 WHERE name=?",
...
def mark_task_incomplete(task_id, db_name):
    ...
    c.execute("UPDATE genip_task SET completed=0 WHERE name=?", (task_id, ))
...
def mark_drmaa_task_completed(task_id, launch_time, start_time, end_time,
    ...
    # Updating
    c.execute("UPDATE genip_task SET launch=?, start=?, end=?, completed=1 "
...
def get_task_runtime(task_id, db_name):
    ...
    c.execute("SELECT start, end FROM genip_task WHERE name=?", (task_id, ))
    r = c.fetchone()
...
def get_all_runtimes(db_name):
    ...
    # Getting the start and end time
    c.execute("SELECT name, start, end FROM genip_task")
    r = c.fetchall()
