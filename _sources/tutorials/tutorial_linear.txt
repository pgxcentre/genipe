...
:ref:`gwip-tut`). These files consist of the imputed genotypes required to
...
:ref:`gwip-tut`). These files greatly resembles the *Plink* ``fam`` file.
...
of processes (the ``--nb-process`` option). This analysis was performed on a
computer with an *Intel(R) Core(TM) i7-3770 CPU @ 3.40GHz* (8 cores) and 16Go
of RAM. The analysis contained 30,000 imputed markers for 2,402 samples. The
red line represent the execution time of the same analysis using *Plink* (which
is simple threaded).
...
Note that the linear regression from *Statsmodels* uses more than one
processor, hence we recommend launching *n/2* processes (where *n* is the
number of processing cores on the machine).
